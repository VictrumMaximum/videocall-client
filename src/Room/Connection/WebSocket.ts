...
import { PeerConnectionsState } from "./PeerConnection";
...
export interface SocketConnectionArgs extends ConnectionArgs {
  peerConnections: PeerConnectionsState;
...
export class SocketConnection {
  ...
  private peerConnections: PeerConnectionsState;
  ...
  constructor(args: SocketConnectionArgs) {
    ...
    this.createPeerConnection = args.createPeerConnection;
      ...
      ws.onmessage = (msg) => {
        ...
        switch (data.type) {
          ...
          case "user-joined-room":
            this.handleUserJoinedRoom(data.source!);
            break;
          case "message":
  ...
  private handleUserJoinedRoom(remoteUserId: string) {
  ...
  private handleMediaOffer(msg: SocketMessage) {
    let myPeerConnection = this.peerConnections[msg.source!].peerConnection;
    if (!myPeerConnection) {
      this.log("Creating peer connection after receiving media offer");
      myPeerConnection = this.createPeerConnection(msg.source!);
    }
    myPeerConnection
  ...
  private handleMediaAnswer(msg: SocketMessage) {
    this.peerConnections[msg.source!].peerConnection.setRemoteDescription(
  ...
  private handleDisconnect(source: string) {
    ...
    if (this.peerConnections[source]) {
      this.peerConnections[source].peerConnection.close();
  ...
  private handleNewICECandidateMsg(msg: SocketMessage) {
    if (this.peerConnections[msg.source!]) {
      this.peerConnections[msg.source!].peerConnection
